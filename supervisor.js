...
${mats||'—'}`
  ...
  // Fotos (comprimir un poco más para asegurar envío)
  const photos=[];
  ...
  // Botones
  $('exportJson').onclick=()=>{ const rep=buildReport(); downloadJSON(rep,(rep.osiId||'OSI')+'_reporte_supervisor.json'); $('hint').textContent='Archivo descargado. Envíalo al encargado por WhatsApp como “Documento”.'; };
  ...
  $('shareReport').onclick=async ()=>{
    ...
    try{
      ...
      if(navigator.canShare && navigator.canShare({files:[file]})){
        await navigator.share({title:'Reporte OSI '+(rep.osiId||''), text:'Adjunto reporte del supervisor (incluye fotos).', files:[file]});
        $('hint').textContent='Compartido. Verifica el chat del Encargado.';
      }else if(navigator.share){
        await navigator.share({title:'Reporte OSI '+(rep.osiId||''), text:'Adjunto reporte del supervisor (se descargará el archivo si no se adjunta).'});
        downloadJSON(rep, file.name);
        $('hint').textContent='El navegador no permite adjuntar archivos. Se descargó el .json: envíalo como documento por WhatsApp.';
      }else{
        downloadJSON(rep, file.name);
        $('hint').textContent='Descargado el .json (tu navegador no soporta compartir). Envíalo como documento por WhatsApp.';
      }
    }catch(_){
      downloadJSON(buildReport(), (payload.id||'OSI')+'_reporte_supervisor.json');
      $('hint').textContent='Compartir falló; se descargó el .json para enviarlo manualmente.';
    }
  };

  $('waText').onclick=()=>{ const txt=`Supervisor — OSI ${payload.id}\n\nResumen:\n${summaryText}\n\nReporte:\n${$('done').value||''}\n\nIncidencias:\n${$('issues').value||''}\n\n(IMPORTANTE: además adjunta el archivo .json para incluir fotos)`; window.open('https://wa.me/?text='+encodeURIComponent(txt),'_blank'); };
})();
