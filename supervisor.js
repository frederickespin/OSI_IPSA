...
(function(){
  ...
  function getPayloadFromURL(){
    ...
    if(!d && location.hash){
      ...
      d = hs.get('d') || h; // compat enlaces antiguos
    }
    if(!d) return null;
    try{ return JSON.parse(b64uToStr(d)); }catch(_){ return null; }
  }
...
${mats||'—'}`
  ...
  function buildReport(){
    return {
      ...
      iniHora: payload.iniHora || '',
      finHora: payload.finHora || '',
